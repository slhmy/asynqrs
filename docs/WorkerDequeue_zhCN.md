...
主循环，可以先执行一次维护 pass，把到期 scheduled/retry 任务 forward 回
pending，并恢复过期 lease；随后调用 `Processor::run_once`，在空队列时
sleep，并由调用方提供 shutdown signal 停止拉取新任务。
...
2. 调用 `WorkerProcessor::run_maintenance`。
3. 对每个队列 forward 到期 scheduled/retry task，并恢复 lease-expired
   active task。
4. 调用 `WorkerProcessor::run_once`。
5. 任务完成、重试、归档或撤销时更新 `ServerRunSummary`。
6. 没有可处理任务时记录 idle poll，并通过 `Sleeper` sleep。
7. shutdown signal 变为 true 后停止拉取新任务并返回 summary。
...
println!(
    "processed={}, forwarded_scheduled={}, recovered_retried={}, idle_polls={}",
    summary.processed(),
    summary.forwarded_scheduled(),
    summary.recovered_retried(),
    summary.idle_polls()
);
```
